...
static ssize_t mopen_read(struct file *f, char *buf, size_t count, loff_t *pos)
{
    ...
    LOG("read - file: %p, read from %p bytes %d; refcount: %d", f, buf_msg, count, module_refcount(THIS_MODULE));
...
static ssize_t mopen_write(struct file *f, const char *buf, size_t count, loff_t *pos)
{
    ...
    LOG("write - file: %p, write to %p bytes %d; refcount: %d", f, buf_msg, count, module_refcount(THIS_MODULE));
